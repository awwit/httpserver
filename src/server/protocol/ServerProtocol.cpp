...
{
	ServerProtocol::ServerProtocol(Socket::Adapter &sock, const ServerSettings &settings, ServerControls &controls) noexcept
	...
	DataVariant::DataReceiver *ServerProtocol::createDataReceiver(const Transfer::request_data *rd, const std::unordered_map<std::string, DataVariant::Abstract *> &variants)
	{
	...
	void ServerProtocol::runApplication(struct Request &req, const ServerApplicationSettings &appSets) const
	{
		std::vector<char> buf;
		buf.reserve(4096);

		if (false == this->packRequestParameters(buf, req, appSets.root_dir) )
		{
			return;
		}
