	...
	bool FormUrlencoded::parse(const std::string &buf, Transfer::request_data *rd, DataReceiver *dr) const
	{
		...
		for (size_t var_pos = 0, var_end = 0; std::string::npos != var_end; var_pos = var_end + 1)
		{
			...
			if (delimiter >= var_end)
			{
				...
				std::string var_name = Utils::urlDecode(buf.substr(var_pos, std::string::npos != var_end ? var_end - var_pos : std::string::npos) );

				// Сохранить параметр с пустым значением
				rd->incoming_data.emplace(std::move(var_name), std::string() );
			}
			else
			{
				// Получить имя параметра
				std::string var_name = Utils::urlDecode(buf.substr(var_pos, delimiter - var_pos) );

				++delimiter;

				// Получить значение параметра
				std::string var_value = Utils::urlDecode(buf.substr(delimiter, std::string::npos != var_end ? var_end - delimiter : std::string::npos) );

				// Сохранить параметр и значение
				rd->incoming_data.emplace(std::move(var_name), std::move(var_value) );
			}
		}
