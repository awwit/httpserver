	...
	bool TextPlain::parse(const std::string &buf, Transfer::request_data *rd, DataReceiver *dr) const
	{
		...
		for (size_t var_pos = 0, var_end = 0; std::string::npos != var_end; var_pos = var_end + 1)
		{
			...
			if (delimiter >= var_end)
			{
				...
				std::string var_name = buf.substr(var_pos, std::string::npos != var_end ? var_end - var_pos : std::string::npos);
			...
			else
			{
				...
				std::string var_value = buf.substr(delimiter, std::string::npos != var_end ? var_end - delimiter : std::string::npos);
